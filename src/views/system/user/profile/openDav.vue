<template>
  <el-form ref="pwdRef" :model="user" :rules="rules" label-width="80px">
    ...
    <el-form-item label="账号" prop="name">
      <el-input v-model="user.name" placeholder="扫码后生成" />
    </el-form-item>
    <el-form-item label="密码" prop="password">
      <el-input v-model="user.password" placeholder="扫码后生成" />
    </el-form-item>
  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button type="primary" @click="handleDialogClose" :loading="loading"
          >生成webDav</el-button
        >
      </span>
    </template>
...
function generateCode() {
  const header = {
    ...
    Authorization:
      'openlist-e31325bb-cc55-4f8c-8c68-b71aa4498929CJSA1uya8ikmtUAP7FBuVi1kcDxd6Kx9zzEqj9yrUqrGCgpQTXt10AMg9MCu3wWK',
  };
  ElMessageBox.alert('生成新的webDav账号、旧的webDav账号将会被删除', '提示', {
    ...
    callback: (action) => {
      if (action === 'confirm') {
        ...
        if (user.davUserId) {
          axios.post(
            'https://play.gssource.com/api/admin/user/delete?id=' +
              user.davUserId,
            {},
            { headers: header },
          );
        ...
        axios
          .post(
            'https://play.gssource.com/api/admin/storage/create',
            {
              ...
              driver: 'QuarkTV',
              addition:
                '{"root_folder_id":"0","order_by":"updated_at","order_direction":"desc","refresh_token":"","device_id":"","query_token":"","link_method":"streaming"}',
            },
            {
              headers: header,
            },
